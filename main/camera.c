...
static esp_err_t set_gpio_config(void)
{
...
{
    ESP_ERROR_CHECK(set_gpio_config());
...
camera_fb_t* camera_get_frame(bool flash)
{
    ...
    // Enable flash led if needed
    if (flash) {
        ESP_ERROR_CHECK(gpio_set_level(GPIO_OUTPUT_IO_4, 1));
    }

    fb = esp_camera_fb_get();

    ESP_ERROR_CHECK(gpio_set_level(GPIO_OUTPUT_IO_4, 0));
